...
import argparse
from stage2.data import get_train_test_set
import os
...
def train(args, train_loader, valid_loader, model, criterion, optimizer, device):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for valid_batch_idx, batch in enumerate(valid_loader):
                ...
                valid_img = valid_img.type(torch.FloatTensor)
                landmark = batch['landmarks']
    ...
    if args.save_model:
        save_model_name = os.path.join(args.save_directory,
                                       'detector_MSELOSS_SGD_(batch_size=10)_(lr=0.00001)_stage2-1-finetune.pt')
        torch.save(model.state_dict(), save_model_name)
...
def show_train_and_val_loss(train_loss_result, val_loss_result, num_epoches):
    ...
    plt.ylabel('loss')
    plt.savefig("train and val loss vs epoches-stage2-1-finetune.jpg")
    plt.close('all')
...
def main_test():
    ...
    if args.phase == 'Train' or args.phase == 'train':
        ...
        model.load_state_dict(torch.load(
            './trained_models/detector_MSELOSS_SGD_(batch_size=10)_(lr=0.00001)_stage2-1.pt'))
        ...
        # predict
        model.load_state_dict(torch.load(
            './trained_models/detector_MSELOSS_SGD_(batch_size=10)_(lr=0.00001)_stage2-1-finetune.pt'))
