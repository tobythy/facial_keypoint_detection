...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(self.prelu2_2(self.bn1(self.conv2_2(x))))
        x = self.prelu3_1(self.conv3_1(x))
        x = self.avg_pool(self.prelu3_2(self.bn2(self.conv3_2(x))))
        x = self.prelu4_1(self.conv4_1(x))
        x = self.prelu4_2(self.bn3(self.conv4_2(x)))
        # flatten
        result = x.view(-1, 4 * 4 * 80)
        result = self.prelu_ip1(self.ip1(result))
        result = self.prelu_ip2(self.bn4(self.ip2(result)))
        # landmarks
...
def show_train_and_val_loss(train_loss_result, val_loss_result, num_epoches):
    ...
    plt.ylabel('loss')
    plt.savefig("train and val loss vs epoches-stage2-2.jpg")
    plt.close('all')
