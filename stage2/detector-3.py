...
def train(args, train_loader, valid_loader, model, criterion, optimizer, device):
    ...
    if args.save_model:
        save_model_name = os.path.join(args.save_directory,
                                       'detector_MSELOSS_SGD_(batch_size=10)_(lr=0.0001)_stage2-3.pt')
...
def predict(img_name, model):
    ...
    if len(faces) > 0:
        ...
        show_image = cv2.resize(show_image, (32, 32))
...
def show_train_and_val_loss(train_loss_result, val_loss_result, num_epoches):
    ...
    plt.ylabel('loss')
    plt.savefig("train and val loss vs epoches-stage2-3.jpg")
    plt.close('all')
...
def main_test():
    ...
    if args.phase == 'Train' or args.phase == 'train':
        ...
        model.load_state_dict(torch.load(
            './trained_models/detector_MSELOSS_SGD_(batch_size=10)_(lr=0.0001)_stage2-3.pt'))
